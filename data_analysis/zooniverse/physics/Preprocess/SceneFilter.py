...
class SceneFilter():
    ...
    def filterScene(self, img, npx, rows, cols):
        ...
        if img[img.shape[0]-1-npx:img.shape[0]-1,
               img.shape[1]-1-npx:img.shape[1]-1].all() != 0.0:
            p0x = img.shape[1]-1-npx; p0y = img.shape[0]-1-npx
            fpx.append((p0y,p0x))
        # Inside the image
    ...
    def perturb(self, img, fpx, npx, dp, thr):
        ...
        for i in range(len(orig)):
            ...
            for p in range(pi.shape[0]):
                check[p] = np.all(img[pi[p][0]:pi[p][0]+npx,
                                      pi[p][1]:pi[p][1]+npx] != 0)
            pi = pi[check,:]
    ...
    def readData(self,date,files,imfiles,fields,thr,plot=False):
        ...
        # Initialise
        cols = fields.copy(); cols.append('image')
            ...
            for s in range(len(files)):
                if all(x in files[s] for x in [datestr, fields[j]]):
                    ...
                    if fields[j] == 'Cloud_Mask_1km':
                        ...
                        cl = np.where(fie<thr); ncl = np.where(fie>=thr)
                        fie[cl[0],cl[1]] = 1; fie[ncl[0],ncl[1]] = 0
                    else:
                        offs = 0; sc = 1; fv = 0;
                        ...
                        fie = (fie - offs)*sc
                    if fields[j] == 'Cloud_Mask_1km':
                        cl = np.where(fie>=0.5); ncl = np.where(fie<0.5)
                        fie[cl[0],cl[1]] = 1; fie[ncl[0],ncl[1]] = 0
        ...
        if plot:
            ...
            axs[5].set_xticks([]); axs[5].set_yticks([])
    ...
    def filterScenes(self):   
        ...
        d0 = np.where(dates == 'A'+str(self.startDate.year)+
                      self.d2yrd(self.startDate))[0][0]
        de = np.where(dates == 'A'+str(self.endDate.year)+
                      self.d2yrd(self.endDate))[0][0]
        ...
        for i in range(d0,de+1):
            ...
            # Set the date
            date = dates[i][1:]; year = int(date[:4]); day  = int(date[4:])
            ...
            for j in range(len(acc)):
                ...
                dfj = pd.DataFrame(index=[ind], 
                                   columns=df.columns)
                rejected = False
                
                # Populate
                for c in dfj.columns:
                    dfj.loc[ind,c] = df.loc[date,c][acc[j][0]:acc[j][0]+self.npx,
                ...
                if not rejected and self.saveOvl:
                    ...
                    if len(acc) > 1:
                        ...
                        dfOvl['iovl'].loc[dfind] = str(date.date())+'-'+ \
                                                   svLab+'-'+str(iovl)
                
                # Find the (lat,lon) of the top left pixel and add to the df
                lon0 = self.lon[0] + \
                       acc[j][1]*(self.lon[1] - self.lon[0])/img.shape[1]
                lat0 = self.lat[1] + \
                       acc[j][0]*(self.lat[0] - self.lat[1])/img.shape[0]
                dfj['lon'] = lon0; dfj['lat'] = lat0
        
                # SAVE TO FOLDER
                if not rejected and self.saveScenes:
                    dfj.to_hdf(self.savePath+'/'+str(date.date())+'-'+svLab+
                               '-'+str(j)+'.h5','sds_filtered',mode='w')
                ...
                if self.plot:
                    ...
                    axs[3].set_title('cloud top height'); 
                    axs[3].set_xticks([]); axs[3].set_yticks([])
                    axs[4].imshow(dfj['Cloud_Water_Path'].values[0],'gray',
                                  vmax=250); 
                    axs[4].set_title('cwp'); 
                    axs[4].set_xticks([]); axs[4].set_yticks([])
                    ...
                    axs[5].set_xticks([]); axs[5].set_yticks([])   
                    if rejected:
                        axs[0].annotate('REJECTED',(0,510),fontsize=15,
                                        zorder=10)
                    plt.show()  
        
        if self.saveOvl:
            dfOvl.to_hdf(self.savePath+'/../ovl-'+svLab+'.h5','overlap',mode='w')
